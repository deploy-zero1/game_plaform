...
import { formatDate, cn, slugify, generateRandomUsername, formatCommentTime, sanitizeText, getRelativeTimeString, buildCanonicalUrl } from "@/lib/utils";
...
export default function GameDetail({ game, relatedGames, outsideGames }: GameDetailProps) {
  ...
  const handleLike = async (commentIndex: number, replyIndex?: number) => {
    // 生成唯一标识以防止重复点赞请求
    const likeId = replyIndex !== undefined 
    ...
    try {
      ...
      // 如果是回复的点赞
      if (replyIndex !== undefined && updatedGame.comments?.[commentIndex]?.replies) {
  ...
  const ogUrl = buildCanonicalUrl(`games/${currentSlug || slugify(game.title)}`, siteConfig.baseUrl);
                ...
                <div className="p-6">
                  ...
                  {game.seo_content && (
                    <div className="mb-8">
                      ...
                      <div 
                        className="game-seo-content text-gray-300 font-roboto bg-background/40 p-6 rounded-lg border border-gray-700 shadow-inner"
                        dangerouslySetInnerHTML={{ __html: game.seo_content }}
                      />
                      <style jsx global>{`
                        .game-seo-content h2 {
                          font-size: 1.25rem;
                        ...
                        <div className="flex-1">
                          <div className="bg-background p-4 rounded-lg">
                            ...
                            <div className="flex items-center mt-3 text-sm">
                              <button 
                                className={`${likingComments.includes(index.toString()) 
                                  ? 'text-accent' 
                                  : 'text-gray-400 hover:text-accent'} transition flex items-center`}
                                onClick={() => handleLike(index)}
                                disabled={likingComments.includes(index.toString())}
                              >
                                ...
                                <div className="flex-1">
                                  <div className="bg-background p-3 rounded-lg">
                                    ...
                                    <div className="flex items-center mt-2 text-sm">
                                      <button 
                                        className={`${likingComments.includes(`${index}-${replyIndex}`) 
                                          ? 'text-accent' 
                                          : 'text-gray-400 hover:text-accent'} transition flex items-center`}
                                        onClick={() => handleLike(index, replyIndex)}
                                        disabled={likingComments.includes(`${index}-${replyIndex}`)}
                                      >
                    ...
                    {outsideGames.map((outsideGame, index) => (
                      ...
                      >
                        ...
                        <div className="p-2 flex-grow flex items-center justify-center">
                          <h4 className="text-white text-xs font-medium text-center line-clamp-1 w-full px-1" title={outsideGame.title}>
                            {outsideGame.title}
                          </h4>
                        </div>
                      </a>
